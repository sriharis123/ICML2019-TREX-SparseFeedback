...
def dense(x, size, name, weight_init=None, bias_init=0, weight_loss_dict=None, reuse=None):
    with tf.compat.v1.variable_scope(name, reuse=reuse):
        assert (len(tf.get_compat.v1.variable_scope().name.split('/')) == 2)
        ...
        if weight_loss_dict is not None:
            ...
            tf.add_to_collection(tf.get_compat.v1.variable_scope().name.split('/')[0] + '_' + 'losses', weight_decay)
...
def kl_div(action_dist1, action_dist2, action_size):
    ...
    return tf.reduce_sum(
        numerator/denominator + tf.math.log(std2) - tf.math.log(std1),reduction_indices=-1)
